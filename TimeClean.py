...
df2['Datetime'] = pd.to_datetime(df2['Date.Time'], format='%d/%m/%Y %H:%M')
...
i = 0
while i < len(df2):
    if df2.at[i, 'uniq.log'] not in sett:
        sett.append(df2.at[i, 'uniq.log'])
    i += 1
# husk at det er andre tidsintervaller for fosen, så må bytte hva som trigger i timeclean for den (ha det som input?)


def timeclean(dataframe):
    month_end = [30, 31]
    temp = 1
    i = 1
    while i < (len(df)-2):
