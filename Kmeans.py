...
df2 = Normalize(df2, ['Haversine'])  # skal jeg ha med begge her?
print(df2)
#print(df2.describe())
...
plt.tight_layout()
#plt.savefig("/Users/ninasalvesen/Documents/Sauedata/Bilder/Master/after cut 4.0/kmeans/tingvoll_elbow_kmeans.png", dpi=500)
plt.show()
# optimal k=6
...
for k in range(1, 20):
    ...
    df2['clusters'] = kmeans.labels_  # .labels_ er det samme som .predict når man putter inn samme datasett i predict
    sse[k] = kmeans.inertia_  # Inertia: Sum of distances of samples to their closest cluster center
plt.figure(figsize=(16, 8))
plt.plot(list(sse.keys()), list(sse.values()))  # gjør dette til ditt eget så det ikke kan plagieres?
plt.title('Elbow method for finding number of clusters in Kmeans++', fontsize=35, pad=15)
...
plt.yticks(fontsize=20)
plt.tight_layout()
#plt.savefig("/Users/ninasalvesen/Documents/Sauedata/Bilder/Master/after cut 4.0/kmeans/fosen_u2018_elbow_kmeans.png", dpi=500)
plt.show()
# optimal k=4
"""
kmeans = KMeans(n_clusters=4)
...
fig2 = px.scatter_3d(df2, x='sin_time', y='cos_time', z='Haversine', color='cluster', opacity=1)
fig2.update_layout(title='3D cluster rep. Fosen')
fig2.update_traces(marker=dict(size=5, line=dict(width=1, color='white')), selector=dict(mode='markers'))
fig2.show()
